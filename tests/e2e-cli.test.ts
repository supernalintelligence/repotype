...
function makeConfigOverrideE2ERepo(): { repoRoot: string; configPath: string } {
  const repoRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "repotype-config-e2e-"),
  );
  const configRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "repotype-config-file-e2e-"),
  );
  fs.mkdirSync(path.join(repoRoot, ".supernal", "features"), {
    recursive: true,
  });
  fs.mkdirSync(path.join(repoRoot, ".supernal", "requirements"), {
    recursive: true,
  });
  fs.mkdirSync(path.join(repoRoot, ".supernal", "traceability"), {
    recursive: true,
  });
...
describe("repotype CLI e2e", () => {
  it("init writes generic preset config", async () => {
    ...
    try {
      const code = await runCLI(["node", "repotype", "init", root]);
      expect(code).toBe(0);
      ...
      // The new UniversalCommand-based CLI always emits JSON.
  ...
  it("validate emits JSON and non-zero for invalid repo", async () => {
    ...
    try {
      // The new UniversalCommand CLI always emits JSON; --json flag removed.
      const code = await runCLI(["node", "repotype", "validate", root]);
      expect(code).toBe(1);
  ...
  it("apply installs configured hooks and status reports state", async () => {
    ...
    try {
      const applyCode = await runCLI(["node", "repotype", "apply", root]);
      ...
      out = "";
      const statusCode = await runCLI(["node", "repotype", "status", root]);
  ...
  it("report command emits markdown and json output", async () => {
    ...
    try {
      ...
      out = "";
      // Use --format json (--json flag removed; CLI always emits structured output).
  ...
  it("validate accepts external config override", async () => {
    ...
    try {
      // --json flag removed; new CLI always emits JSON.
      const code = await runCLI([
        "node",
        "repotype",
        "validate",
