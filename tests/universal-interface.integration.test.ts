...
describe("universal-command testing integration", () => {
  ...
  it("runs repotype report command via universal test harness", async () => {
    ...
    expect(result.success).toBe(true);
    // cli.format renders the report JSON directly when --format json is passed;
    // the top-level keys are the ComplianceReport fields, not a wrapper object.
    expect(result.stdout).toContain('"generatedAt"');
