...
function resolveRepoRoot(targetRoot, configPath) {
  ...
  const rel = path7.relative(configDir, targetRoot);
  const targetInsideConfigDir = rel === "" || !rel.startsWith("..") && !path7.isAbsolute(rel);
          ...
          if (absReqFile.startsWith(ws.subtreeRoot + path7.sep) || absReqFile === ws.subtreeRoot) {
            const childRequires = (childConfig.folders ?? []).some(
              (cf) => (cf.requiredFiles ?? []).some(
                (rf) => path7.resolve(ws.subtreeRoot, rf) === absReqFile
              )
            );
            if (!childRequires) {
              conflicts.push({
...
var CrossFileRuleAdapter = class {
  ...
  supports(filePath, context) {
    ...
    return rules.some(
      (r) => r.kind === "cross_reference" && r.field && r.target && minimatch(path9.relative(context.repoRoot, filePath).replace(/\\/g, "/"), r.sourceGlob, { dot: true })
    );
  }
  async validate(filePath, context) {
    const diagnostics = [];
...
function isWithinTargetScope(ruleTargetDir, targetRoot) {
  const rel = path12.relative(targetRoot, ruleTargetDir);
  const targetIsAncestorOrEqual = rel === "" || !rel.startsWith("..") && !path12.isAbsolute(rel);
  if (targetIsAncestorOrEqual) return true;
  const relInverse = path12.relative(ruleTargetDir, targetRoot);
  return relInverse !== "" && !relInverse.startsWith("..") && !path12.isAbsolute(relInverse);
}
  ...
  async validate(_filePath, context) {
    ...
    for (const rule of folderRules) {
      ...
      for (const targetDir of targets) {
        ...
        const entries = fs11.readdirSync(targetDir, { withFileTypes: true }).filter(
          (entry) => !ignoreMatcher.isIgnored(path12.join(targetDir, entry.name))
        );
...
var CronRegistryDriftAdapter = class {
  ...
  computeDrift(monorepoRoot) {
    ...
    const boardsDir = path20.join(monorepoRoot, "packages", "boards");
    let boardDirs;
    try {
    ...
    for (const boardId of boardDirs) {
      ...
      const yamlPath = path20.join(boardsDir, boardId, "board.yaml");
...
function installWatcher(options) {
  const target = path26.resolve(options.target);
  const queueDir = path26.resolve(options.queueDir);
  const logFile = path26.resolve(options.logFile);
  fs26.mkdirSync(path26.dirname(logFile), { recursive: true });
  fs26.mkdirSync(queueDir, { recursive: true });
...
function normalizeOperations(target) {
  ...
  const normalized = {
    ...
    watcher: {
      ...
      queueDir: path27.resolve(repoRoot, config.operations?.watcher?.queueDir ?? "sort_queue"),
      minErrors: config.operations?.watcher?.minErrors ?? 3,
      logFile: path27.resolve(repoRoot, config.operations?.watcher?.logFile ?? ".repotype/logs/watcher.log")
    }
  };
  return {
    repoRoot,
...
function readLastCleanupEntry(queueDir) {
  const logPath = path27.join(queueDir, "cleanup-log.jsonl");
  if (!fs27.existsSync(logPath)) {
    return { found: false };
  }
  const lines = fs27.readFileSync(logPath, "utf8").split("\n").map((line) => line.trim()).filter(Boolean);
  if (lines.length === 0) {
    return { found: false };
  }
  try {
