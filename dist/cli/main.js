...
function loadSchema(repoRoot, schemaRef) {
  const schemaPath = path2.resolve(repoRoot, schemaRef);
  const schemaRaw = fs4.readFileSync(schemaPath, "utf8");
...
var FileSchemaAdapter = class {
  ...
  async validate(filePath, context) {
    ...
    const schemaPath = path2.resolve(context.repoRoot, schemaRef);
...
async function runCLI(argv) {
  ...
  program.command("validate").argument("[target]", "file or directory to validate", ".").option("--json", "emit machine-readable JSON", false).option("--config <path>", "explicit config file path (repotype.yaml)", "").action(async (target, options) => {
    const result = await validatePath(target, options.config || void 0);
  ...
  program.command("fix").argument("[target]", "file or directory to validate/fix", ".").option("--config <path>", "explicit config file path (repotype.yaml)", "").action(async (target, options) => {
