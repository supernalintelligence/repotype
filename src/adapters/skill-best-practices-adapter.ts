 ...
 *  3. `name:` field present, equals the parent directory name, and matches
 *     `^[a-z0-9]+(-[a-z0-9]+)*$` (lowercase/digits/hyphens, no consecutive,
 *     leading, or trailing hyphen). Violation: error.
...
/** Slug shape for a skill name: lowercase/digits, single hyphens between groups. */
      ...
      if (name === undefined || name === null || name === '') {
        ...
        if (!SKILL_NAME_RE.test(name)) {
          diagnostics.push({
            ...
            message: `SKILL.md 'name' ("${name}") must be lowercase letters, digits, and single hyphens only (no leading, trailing, or consecutive hyphens).`,
            severity: 'error',
            file: filePath,
            ruleId: this.id,
            details: { skill: skillDir, name },
